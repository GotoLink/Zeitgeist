...
public class zei_ModelToter extends ModelQuadruped {
	// fields
	ModelRenderer flap;

	// ModelRenderer body;
	public zei_ModelToter() {
		super(6, 1);
		textureWidth = 64;
		textureHeight = 32;
	...
	public void setLivingAnimations(EntityLiving living, float par2,
			float par3, float par4) {
		zei_EntityTote tote = (zei_EntityTote) living;
		tote.ango += tote.dir;
		if (tote.ango > 90) {
			tote.ango = 90;
		} else if (tote.ango < 20) {
			tote.ango = 20;
		}
		flap.rotateAngleX = (float) (Math.PI * tote.ango / 180.0);
	}
