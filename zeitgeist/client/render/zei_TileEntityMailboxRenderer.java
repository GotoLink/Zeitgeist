...
public class zei_TileEntityMailboxRenderer extends TileEntitySpecialRenderer {
	static float ss = 6f / 256f;
	static float ee = 10f / 256f;

	public void renderTurnAt(zei_TileEntityMailbox enti, double x, double y,
			double z, float par8) {
		int i;
		...
		/*
		 * GL11.glRotatef(u, 0, 0, 1); GL11.glRotatef(j, 0, 1, 0);
		 * GL11.glRotatef(enti.ang, 1, 0, 0);
		 * 
		 * enti.ang += dir;
		 * 
		 * if (enti.ang > 360) { enti.ang = 0; } else if (enti.ang < 0) {
		 * enti.ang = 360; }
		 */
	...
	public void renderTileEntityAt(TileEntity par1TileEntity, double par2,
		...
		renderTurnAt((zei_TileEntityMailbox) par1TileEntity, par2, par4, par6,
				par8);
	}
}
