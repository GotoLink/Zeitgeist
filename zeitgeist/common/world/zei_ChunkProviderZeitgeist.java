...
public class zei_ChunkProviderZeitgeist implements IChunkProvider {
	/** RNG. */
	private Random rand;
	...
	public zei_ChunkProviderZeitgeist(World par1World, long par2, boolean par4) {
		...
		for (int k1 = 0; k1 < 16; k1++) {
			for (int l2 = 0; l2 < 16; l2++) {
				...
				if (worldObj.isBlockFreezable(k1 + i, l3 - 1, l2 + j)) {
					worldObj.setBlock(k1 + i, l3 - 1, l2 + j,
							Block.ice.blockID);
				}
				if (worldObj.canSnowAt(k1 + i, l3, l2 + j)) {
					worldObj.setBlock(k1 + i, l3, l2 + j,
							Block.snow.blockID);
				}
			}
		}
