...
public class zei_WorldGenEmpire extends WorldGenerator {
	ArrayList<ArrayList<Integer>> chests = new ArrayList<ArrayList<Integer>>();
	public int M = 16;
	public int set = 0;
	public int W;

	public zei_WorldGenEmpire() {
	}
	...
	private void cavernize(World world, Random random, int height, int width, int length, int i, int j, int k) {
		 ...
		 */
		/*
		 * for(int z=0;z<=length;z++){ for(int y=1;y<=he;y++){ for(int
		 * x=0;x<=width;x++){ world.setBlock(i+(x-w2), j+y, k+(z-l2), 0); } } }
		 * for(int z=0;z<=length;z++){ for(int y=he;y<=newheight;y++){ for(int
		 * x=0;x<=width;x++){ //System.out.println(newheight+" : "+y);
		 * if(random.nextInt(1+newheight-y)!=0){ world.setBlock(i+(x-w2), j+y,
		 * k+(z-l2), 0); } } } }
		 */
	...
	public boolean generate(World world, Random random, int i, int j, int k) {
		...
		/*
		 * if(world.getBlockId(i,j,k)==AutomatonLogger.tech){ return true; }
		 */
		if (world.getBlockId(i, j, k) != zei_Ids.sky) {
		...
		/*
		 * if(bb1==AutomatonLogger.sky &&
		 * world.getBlockId(i,j-1,(k+w1))==AutomatonLogger.tech){
		 * mustBeInside=true; bo[0]=false; } if(bb2==AutomatonLogger.sky &&
		 * world.getBlockId(i,j-1,(k-w1))==AutomatonLogger.tech){
		 * mustBeInside=true; bo[1]=false; } if(bb3==AutomatonLogger.sky &&
		 * world.getBlockId((i+w1),j-1,k)==AutomatonLogger.tech){
		 * mustBeInside=true; bo[2]=false; } if(bb4==AutomatonLogger.sky &&
		 * world.getBlockId((i-w1),j-1,k)==AutomatonLogger.tech){
		 * mustBeInside=true; bo[3]=false; }
		 */
		bo[0] = (bb1 != zei_Ids.sky);
		bo[1] = (bb2 != zei_Ids.sky);
		bo[2] = (bb3 != zei_Ids.sky);
		bo[3] = (bb4 != zei_Ids.sky);
		 ...
		 * AutomatonLogger.sky,1); } }
		 */
		if (bo[0]) {
			world.setBlock(i, j - 1, k - 1, zei_Ids.tech);
		}
		if (bo[1]) {
			world.setBlock(i, j - 1, k + 1, zei_Ids.tech);
		}
		if (bo[2]) {
			world.setBlock(i - 1, j - 1, k, zei_Ids.tech);
		}
		if (bo[3]) {
			world.setBlock(i + 1, j - 1, k, zei_Ids.tech);
		}
	...
	private ItemStack pickCheckLootItem(Random random) {
		...
		if (i == 5) {
			// return null;//new ItemStack(zei_Ids.crystal, random.nextInt(4) +
			// 1,0);
		}
		if (i == 7) {
	...
	private void struct(World world, Random random, boolean[] boo, int height, int width, int length, int i, int j, int k, boolean BOOL) {
		...
		if (zup) {
			...
			for (int x = 0; x <= w3; x++) {
				world.setBlock((x - w4) + i, j + h5, k + l2 - zh, zei_Ids.sky, 1, 2);
			}
			for (int x = 0; x <= w3; x++) {
		...
		if (xup) {
			...
			for (int z = 0; z <= l3; z++) {
				world.setBlock(i + w2 - zh, j + h5, k + (z - l4), zei_Ids.sky, 1, 2);
			}
			for (int z = 0; z <= l3; z++) {
				for (int y = h5 + 1; y <= height; y++) {
