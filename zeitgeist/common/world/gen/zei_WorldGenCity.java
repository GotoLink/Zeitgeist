...
public class zei_WorldGenCity extends WorldGenerator {
	public int C = 7;
	public boolean cell[][] = new boolean[C][C];
	public int W;
	ArrayList<ArrayList<Integer>> chests = new ArrayList<ArrayList<Integer>>();

	public zei_WorldGenCity() {
	}

	public void addTunnels(World world, int i, int j, int k) {
		int bb = 4;
		int ww = (W / 2);
		if (world.getBlockId(i + W, j, k) == zei_Ids.tech) {
		...
		if (world.getBlockId(i, j, k + W) == zei_Ids.tech) {
	...
	public boolean generate(World world, Random random, int i, int j, int k) {
		...
		for (int ii = 0; ii < C; ii++) {
			for (int jj = 0; jj < C; jj++) {
				if (cell[ii][jj]) {
					...
					for (int z = 0; z <= m; z++) {
						for (int x = 0; x <= m; x++) {
							world.setBlock(i + (x - m22) + m * (ii - m2), j, k + (z - m22) + m * (jj - m2), zei_Ids.sky, 1, 3);
						}
					}
				}
			}
		...
		int r = m * m2;
		world.setBlock(i, j, k, zei_Ids.tech);
	...
	private void cavernize(World world, Random random, int height, int width, int length, int i, int j, int k) {
		...
		for (int z = 0; z <= length; z++) {
			for (int x = 0; x <= width; x++) {
				world.setBlock(i + (x - w2), j, k + (z - l2), zei_Ids.arch, 1, 3);
			}
		}
		for (int z = 0; z < length; z++) {
			for (int y = 1; y <= he; y++) {
		...
		if (RR > 2) {
			...
			default:
				...
				((zei_WorldGenBigFakeTree) obj).boo = false;
				break;
			}
	...
	private void struct(World world, Random random, boolean[] boo, int height, int width, int length, int i, int j, int k) {
		...
		if (random.nextInt(4) != 0) {
			...
			/*
			 * else if(random.nextInt(5)==0){ world.setBlockAndMetadata(i, j+1,
			 * k, zei_Ids.deployer,random.nextInt(2)+1); }
			 */
		}
		/*
		 * switch(O){ case
