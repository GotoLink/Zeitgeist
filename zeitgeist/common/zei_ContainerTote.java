...
public class zei_ContainerTote extends Container {
	private IInventory lowerChestInventory;
	private int numRows;

	public zei_ContainerTote(IInventory par1IInventory, IInventory par2IInventory) {
		...
		for (int j = 0; j < 3; j++) {
			for (int i1 = 0; i1 < 3; i1++) {
				addSlotToContainer(new Slot(par2IInventory, i1 + j * 3, 62 + i1 * 18, 17 + j * 18));
			}
		}
		for (int k = 0; k < 3; k++) {
			for (int j1 = 0; j1 < 9; j1++) {
				addSlotToContainer(new Slot(par1IInventory, j1 + k * 9 + 9, 8 + j1 * 18, 102 + k * 18 + i));
			}
		}
		for (int l = 0; l < 9; l++) {
