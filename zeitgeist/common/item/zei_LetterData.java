...
public class zei_LetterData extends WorldSavedData {
	/*
	...
	public zei_LetterData(String par1Str) {
		super(par1Str);
	...
	public void readFromNBT(NBTTagCompound par1NBTTagCompound) {
		/*
		 * dimension = par1NBTTagCompound.getByte("dimension"); xCenter =
		 * par1NBTTagCompound.getInteger("xCenter"); zCenter =
		 * par1NBTTagCompound.getInteger("zCenter"); scale =
		 * par1NBTTagCompound.getByte("scale");
		 * 
		 * if (scale < 0) { scale = 0; }
		 * 
		 * if (scale > 4) { scale = 4; }
		 * 
		 * short word0 = par1NBTTagCompound.getShort("width"); short word1 =
		 * par1NBTTagCompound.getShort("height");
		 * 
		 * if (word0 == 128 && word1 == 128) { colors =
		 * par1NBTTagCompound.getByteArray("colors"); } else { byte abyte0[] =
		 * par1NBTTagCompound.getByteArray("colors"); colors = new byte[16384];
		 * int i = (128 - word0) / 2; int j = (128 - word1) / 2;
		 * 
		 * for (int k = 0; k < word1; k++) { int l = k + j;
		 * 
		 * if (l < 0 && l >= 128) { continue; }
		 * 
		 * for (int i1 = 0; i1 < word0; i1++) { int j1 = i1 + i;
		 * 
		 * if (j1 >= 0 || j1 < 128) { colors[j1 + l * 128] = abyte0[i1 + k *
		 * word0]; } } } }
		 */
		message = par1NBTTagCompound.getString("message");
		header = par1NBTTagCompound.getString("header");
		to = par1NBTTagCompound.getString("to");
		from = par1NBTTagCompound.getString("from");
	}
	...
	public void writeToNBT(NBTTagCompound par1NBTTagCompound) {
		 ...
		 */
		par1NBTTagCompound.setString("message", message);
		par1NBTTagCompound.setString("header", header);
		par1NBTTagCompound.setString("to", to);
		par1NBTTagCompound.setString("from", from);
	}
	/*
