...
public class zei_BlockFlowing2 extends zei_BlockFluid2 {
	/**
	...
	protected zei_BlockFlowing2(int par1, Material par2Material) {
	...
	private void updateFlow(World par1World, int par2, int par3, int par4) {
		...
		par1World.setBlock(par2, par3, par4, blockID + 1, i, 3);
	...
	public void updateTick(World par1World, int par2, int par3, int par4,
		...
		if (i > 0) {
			...
			if (k != i) {
				...
				if (i < 0) {
					...
					par1World
							.setBlockMetadataWithNotify(par2, par3, par4, i, 3);
		...
		if (liquidCanDisplaceBlock(par1World, par2, par3 - 1, par4)) {
			/*
			 * if (par1World.getBlockMaterial(par2, par3 - 1, par4) ==
			 * Material.water) { par1World.setBlockWithNotify(par2, par3 - 1,
			 * par4, Block.waterMoving.blockID);
			 * triggerLavaMixEffects(par1World, par2, par3 - 1, par4); return; }
			 * 
			 * if ( par1World.getBlockMaterial(par2, par3 - 1, par4) ==
			 * Material.lava) { par1World.setBlockWithNotify(par2, par3 - 1,
			 * par4, Block.lavaMoving.blockID); triggerLavaMixEffects(par1World,
			 * par2, par3 - 1, par4); return; }
			 */
			if (i >= 8) {
				par1World.setBlock(par2, par3 - 1, par4, blockID, i, 3);
			} else {
				par1World.setBlock(par2, par3 - 1, par4, blockID, i + 8, 3);
			}
		} else if (i >= 0
			...
			if (par1World.getBlockMaterial(par2, par3 - 1, par4) == Material.lava) {
				par1World.setBlock(par2, par3 - 1, par4, 3);
				triggerLavaMixEffects(par1World, par2, par3 - 1, par4);
				return;
			}
			/*
			 * if (par1World.getBlockMaterial(par2+1, par3 , par4) ==
			 * Material.water ||
			 * 
			 * par1World.getBlockMaterial(par2-1, par3 , par4) ==
			 * Material.water|| par1World.getBlockMaterial(par2, par3 , par4+1)
			 * == Material.water|| par1World.getBlockMaterial(par2, par3 ,
			 * par4-1) == Material.water
			 * 
			 * ) { par1World.setBlockWithNotify(par2, par3 , par4,
			 * Block.waterStill.blockID); triggerLavaMixEffects(par1World, par2,
			 * par3 - 1, par4); return; }
			 */
	...
	private void flowIntoBlock(World par1World, int par2, int par3, int par4,
		 ...
		 * else if(par1World.getBlockMaterial(par2, par3,
		 * par4)==Material.water){ par1World.setBlockAndMetadataWithNotify(par2,
		 * par3, par4, Block.leaves.blockID, par5); }
		 */
	}
	...
	private boolean blockBlocksFlow(World par1World, int par2, int par3,
			int par4) {
		int i = par1World.getBlockId(par2, par3, par4);
		if (i == Block.doorWood.blockID || i == Block.doorIron.blockID
				|| i == Block.signPost.blockID || i == Block.ladder.blockID
				|| i == Block.reed.blockID || (i >= 8 && i <= 11)) {
			return true;
		}
		if (i == 0) {
			return false;
