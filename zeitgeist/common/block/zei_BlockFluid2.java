...
public abstract class zei_BlockFluid2 extends Block {
	protected zei_BlockFluid2(int par1, Material par2Material) {
		// water 205 lava 237
		super(par1, 237, par2Material); // 6
		float f = 0.0F;
		float f1 = 0.0F;
	...
	public int colorMultiplier(IBlockAccess par1IBlockAccess, int par2,
			int par3, int par4) {
		/*
		 * if (blockMaterial == Material.water) { int i = 0; int j = 0; int k =
		 * 0;
		 * 
		 * for (int l = -1; l <= 1; l++) { for (int i1 = -1; i1 <= 1; i1++) {
		 * int j1 = par1IBlockAccess.getBiomeGenForCoords(par2 + i1, par4 +
		 * l).waterColorMultiplier; i += (j1 & 0xff0000) >> 16; j += (j1 &
		 * 0xff00) >> 8; k += j1 & 0xff; } }
		 * 
	...
	public int getRenderType() {
		return zei_Ids.liquidRenderId; // 4
	}
	 ...
	 */
	private Vec3 getFlowVector(IBlockAccess par1IBlockAccess, int par2,
			int par3, int par4) {
	...
	public void velocityToAddToEntity(World par1World, int par2, int par3,
			int par4, Entity par5Entity, Vec3 par6Vec3D) {
		Vec3 vec3d = getFlowVector(par1World, par2, par3, par4);
	...
	public int tickRate() {
		...
		return 8;// blockMaterial != Material.lava ? 0 : 30;
	}
	...
	public int getRenderBlockPass() {
		return 0;// blockMaterial != Material.water ? 0 : 1;
	}
	...
	public static double func_293_a(IBlockAccess par0IBlockAccess, int par1,
		...
		// {
		Vec3 vec3d = ((zei_BlockFluid2) Block.blocksList[zei_Ids.testLiquid])
				.getFlowVector(par0IBlockAccess, par1, par2, par3);
		// }

		// if (par4Material == Material.lava)
		// {
		// vec3d =
		// ((zei_BlockFluid2)Block.lavaMoving).getFlowVector(par0IBlockAccess,
		// par1, par2, par3);
		// }
	...
	private void checkForHarden(World par1World, int par2, int par3, int par4) {
		...
		if (blockMaterial == zei_Blocks.liqui) {
			boolean flag = false;
